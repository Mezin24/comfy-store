...
import { convertToDollars, generateAmountOptions } from '../utils';
import { editItem, removeItem } from '../features/cart/cartSlice';

export const CartItem = ({ cartItem }) => {
  const { cartId, title, image, price, company, productColor, amount } =
    cartItem;
  ...
  return (
    <article className='mb-12 flex flex-col gap-y-4 sm:flex-row flex-wrap border-b border-base-300 pb-6 last:border-b-0'>
      ...
      <p className='font-medium sm:ml-auto'>{convertToDollars(price)}</p>
    </article>
  );
};
